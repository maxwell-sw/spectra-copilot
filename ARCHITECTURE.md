...
> 状态：本地原型 v0.2。本文同时记录已实现行为与后续公开产品规划；“公开版”“下一阶段”等表述均不是当前已实现功能。
...
| --- | --- | --- |
| 公开网站 | 选择文件、多选文件或选择一个文件夹 | 仅用户在浏览器选择的文件/文件夹 |
| 本地桌面连接器 | 安装桌面程序后，在系统权限窗口选择目录 | 仅用户授予的目录，例如 Desktop/实验数据 |

普通网页不能像 Codex 那样扫描整台电脑；需要 Codex 式体验时，网页应配合一个用户安装的本地连接器。连接器只监听 `localhost`，每次扩大目录范围都由用户确认。

当前仓库是本地原型：服务固定监听 `127.0.0.1`，桌面搜索默认范围为当前用户的 `Desktop` 目录。公开网页版本必须改为显式的文件/文件夹授权，不能沿用这一默认范围。
...
4. **模型决策与工具循环**：模型在每轮选择“读摘要、扫描原始行、计算、画图、生成 HTML”之一；工具返回 JSON 事实，模型再决定继续或结束。默认最多 5 轮，避免无止境花费。
